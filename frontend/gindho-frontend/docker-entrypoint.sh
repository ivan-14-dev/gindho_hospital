...
{
  "gatewayUrl": "${VITE_GATEWAY_URL:-http://localhost:8080}",
  "keycloakUrl": "${VITE_KEYCLOAK_URL:-http://localhost:8081}",
...
if [ -n "$GATEWAY_URL" ]; then
    sed -i "s|http://localhost:8080|${GATEWAY_URL}|g" /usr/share/nginx/html/index.html 2>/dev/null || true
fi
