...
import { TrendingUp, AlertTriangle, Virus, Activity } from 'lucide-react';
...
export function EpidemiologyView() {
  ...
  const { data: metrics = {} } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get('/analytics-service/epidemiology-metrics', { params: { startDate: dateRange.start, endDate: dateRange.end, period, ...filters } });
      return response.data || {};
    },
  });

  const kpiData = [
    { title: 'Cas Actifs', value: metrics.activeCases || 0, change: -2.1, trend: 'down' as const, icon: Virus, color: 'text-red-600' },
  ...
  return (
    <div className="p-6 space-y-6">
      <div><h1 className="text-3xl font-bold flex items-center gap-2"><Virus className="h-8 w-8 text-red-600" />Analyse Épidémiologie</h1><p className="text-muted-foreground mt-1">Surveillance des maladies</p></div>
