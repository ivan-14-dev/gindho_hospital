...
export function PatientsView() {
  ...
  const { data: metrics = {} } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.get(`/analytics-service/patients-metrics?${params.toString()}`);
  ...
  const { data: chartData = {} } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.get(`/analytics-service/patients-charts?${params.toString()}`);
      return response.data || { ageDistribution: [], genderDistribution: [], admissionTrend: [] };
    },
  });
      const response = await apiClient.get(`/analytics-service/patients-charts?${params.toString()}`);
      return response.data || { ageDistribution: [], genderDistribution: [], admissionTrend: [] };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {chartData.ageDistribution?.length > 0 && (<BarChartComponent title="Distribution par Âge" data={chartData.ageDistribution} icon={<Calendar className="h-4 w-4 text-orange-600" />} height={300} />)}
        {chartData.genderDistribution?.length > 0 && (<PieChartComponent title="Répartition par Sexe" data={chartData.genderDistribution} icon={<Users className="h-4 w-4 text-pink-600" />} height={300} />)}
        {chartData.admissionTrend?.length > 0 && (<LineChartComponent title="Tendance Admissions" data={chartData.admissionTrend} icon={<TrendingUp className="h-4 w-4 text-green-600" />} height={300} />)}
      </div>
    </div>
  );
}
