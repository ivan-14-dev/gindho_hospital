...
# Variable d'environnement HOME autorisée mais non utilisée ici
export HOME=/tmp
...
# Appeler l'action de configuration des données si elle est définie
if [[ -n "${IDENTITY_SERVICE_SETUP:-}" ]]; then
  echo "-> Exécution de IDENTITY_SERVICE_SETUP=$IDENTITY_SERVICE_SETUP"
  # Utiliser spring boot run pour vérifier si c'est une classe principale ou un jar
